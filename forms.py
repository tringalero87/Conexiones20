...
class ProjectForm(FlaskForm):
    ...
    nombre = StringField(
        ...
        render_kw={
            "placeholder": "Ej: Remodelación Edificio Central"})
...
class ProfileForm(FlaskForm):
    ...
    email_notif_estado = BooleanField(
        'Recibir notificaciones por email sobre cambios de estado de conexiones')
    ...
    per_page = IntegerField(
        ...
        validators=[
            ...
            NumberRange(
                min=5,
                max=50,
                message="El valor debe ser un número entre 5 y 50.")],
...
class ReportForm(FlaskForm):
    ...
    nombre = StringField(
        ...
        render_kw={
            "placeholder": "Ej: Reporte Mensual de Conexiones Aprobadas"})

    descripcion = TextAreaField(
        'Descripción', validators=[
            Optional(), Length(
                max=500)], render_kw={
            "placeholder": "Añade una breve descripción del propósito de este reporte..."})
    ...
    realizador_id = SelectField(
        'Realizador',
        coerce=int,
        validators=[
            Optional()])
    fecha_inicio = DateField(
        'Fecha de Inicio', validators=[
    ...
    output_format = RadioField(
        ...
        validators=[
            DataRequired(
                message="Debes seleccionar un formato de salida.")])

    programado = BooleanField('Programar envío por email')
    frecuencia = SelectField(
        'Frecuencia', choices=[
            ('diaria', 'Diaria'), ('semanal', 'Semanal'), ('mensual', 'Mensual')], validators=[
            Optional()])
    destinatarios = TextAreaField(
        'Destinatarios (emails separados por coma)',
        render_kw={
            "placeholder": "ejemplo1@heptapro.com, ejemplo2@heptapro.com"},
        validators=[
            Optional()])
    ...
    def validate_destinatarios(self, field):
        ...
        if self.programado.data:
            if not field.data:
                raise ValidationError(
                    'Este campo es obligatorio si el reporte está programado.')

            emails = [e.strip() for e in field.data.split(',') if e.strip()]
            if not emails:
                raise ValidationError(
                    'Debe ingresar al menos un correo electrónico si el reporte está programado.')
            ...
            for email in emails:
                if not email_regex.match(email):
                    raise ValidationError(
                        f'El correo electrónico "{email}" no es válido.')
...
class ComputosReportForm(FlaskForm):
    ...
    proyecto_id = SelectField(
        'Proyecto', coerce=int, validators=[
            DataRequired(
                message="Debe seleccionar un proyecto.")])
    fecha_inicio = DateField(
        'Fecha de Inicio (Opcional)', validators=[
            Optional()], format='%Y-%m-%d')
    fecha_fin = DateField(
...
class ConnectionForm(FlaskForm):
    ...
    """
    perfil_1 = StringField(
        'Perfil 1', validators=[
            DataRequired(
                message="El Perfil 1 es obligatorio.")])
