...
def test_current_realizador_can_delete_files(client, app, auth):
    ...
    with app.app_context():
        ...
        cursor.execute(
            "INSERT INTO usuarios (username, password_hash, nombre_completo, email, activo) VALUES ('realizador1', ?, 'Realizador One', 'r1@test.com', 1)", (password_hash,))
        realizador1_id = cursor.lastrowid
        cursor.execute(
            "INSERT INTO usuarios (username, password_hash, nombre_completo, email, activo) VALUES ('realizador2', ?, 'Realizador Two', 'r2@test.com', 1)", (password_hash,))
        ...
        cursor.execute("INSERT INTO usuario_roles (usuario_id, rol_id) VALUES (?, ?)",
                       (realizador1_id, realizador_role_id))
        cursor.execute("INSERT INTO usuario_roles (usuario_id, rol_id) VALUES (?, ?)",
                       (realizador2_id, realizador_role_id))

        cursor.execute(
            "INSERT INTO proyectos (nombre, descripcion) VALUES ('Test Project', 'Desc')")
    ...
    with client.session_transaction() as session:
        assert 'Archivo \'Memoria de Calculo\' subido con éxito.' in [
            msg for cat, msg in session['_flashes']]

    with app.app_context():
        db = get_db()
        cursor = db.cursor()
        cursor.execute(
            "SELECT id FROM archivos WHERE conexion_id = ?", (connection_id,))
    ...
    client.post(f'/conexiones/{connection_id}/asignar',
                data={'username_a_asignar': 'realizador2'}, follow_redirects=True)
    auth.logout()
    ...
    response = client.post(
        # Changed to False
        f'/conexiones/{connection_id}/eliminar_archivo/{file_id}', follow_redirects=False)
    assert response.status_code == 302
...
def test_solicitante_cannot_edit_other_users_connection(client, app, auth):
    ...
    with app.app_context():
        ...
        cursor.execute("INSERT INTO usuario_roles (usuario_id, rol_id) VALUES (?, ?)",
                       (solicitante1_id, solicitante_role_id))
        cursor.execute("INSERT INTO usuario_roles (usuario_id, rol_id) VALUES (?, ?)",
                       (solicitante2_id, solicitante_role_id))

        # Create a project
        cursor.execute(
            "INSERT INTO proyectos (nombre, descripcion) VALUES ('Project Edit Test', 'Desc')")
        project_id = cursor.lastrowid
        ...
        cursor.execute(
            ...
            """,
            ('CONN-EDIT-TEST', project_id,
             solicitante1_id, '{"Perfil 1": "IPE300"}')
        )
        connection_id = cursor.lastrowid
        db.commit()
    ...
    with app.app_context():
        ...
        cursor = db.cursor()
        cursor.execute(
            "SELECT descripcion FROM conexiones WHERE id = ?", (connection_id,))
...
def test_upload_dotfile_is_rejected(client, app, auth):
    ...
    with app.app_context():
        ...
        realizador_role_id = cursor.fetchone()['id']
        cursor.execute("INSERT INTO usuario_roles (usuario_id, rol_id) VALUES (?, ?)",
                       (user_id, realizador_role_id))
        cursor.execute(
            "INSERT INTO proyectos (nombre, descripcion) VALUES ('Dotfile Test Project', 'Desc')")
        project_id = cursor.lastrowid
        cursor.execute(
            "INSERT INTO conexiones (codigo_conexion, proyecto_id, tipo, subtipo, tipologia, solicitante_id, realizador_id, estado) VALUES (?, ?, 'Test', 'Test', 'Test', ?, ?, 'EN_PROCESO')",
    ...
    # Check for absence of success message
    assert b'subido con xito' not in response.data
...
def test_computos_metricos_with_duplicate_profiles(client, app, auth):
    ...
    with app.app_context():
        ...
        # Create a project
        cursor.execute(
            "INSERT INTO proyectos (nombre, descripcion) VALUES ('Computos Test Project', 'Desc')")
        project_id = cursor.lastrowid
        ...
        cursor.execute(
            ...
            """,
            ('COMPUTOS-BUG-TEST', project_id,
             admin_user['id'], json.dumps(detalles_iniciales))
        )
        connection_id = cursor.lastrowid
        db.commit()
    ...
    with app.app_context():
        ...
        cursor = db.cursor()
        cursor.execute(
            "SELECT detalles_json FROM conexiones WHERE id = ?", (connection_id,))
        conexion = cursor.fetchone()
...
def test_reporte_computos_shows_correct_data(client, app, auth):
    ...
    with app.app_context():
        ...
        admin_user = cursor.fetchone()
        cursor.execute(
            "INSERT INTO proyectos (nombre, descripcion) VALUES ('Reporte Computos Test', 'Desc')")
        project_id = cursor.lastrowid
        ...
        cursor.execute(
            ...
            VALUES (?, ?, 'MOMENTO', 'VIGA-COLUMNA (ALA)', 'T1', ?, 'SOLICITADO', ?)
            """,
            ('REPORTE-COMPUTOS-TEST', project_id,
             admin_user['id'], json.dumps(detalles_iniciales))
        )
        connection_id = cursor.lastrowid
        db.commit()
...
def test_eliminar_archivo_con_path_traversal(client, app, auth):
    ...
    try:
        with app.app_context():
            ...
            cursor.execute(
                "INSERT INTO proyectos (nombre, descripcion) VALUES ('Path Traversal Test', 'Desc')")
            project_id = cursor.lastrowid
            cursor.execute(
            ...
            malicious_filename = "../../sensitive_file.txt"
            cursor.execute(
...
def test_conexion_report_does_not_crash(client, app, auth):
    """
    Tests that generating a connection report doesn't crash due to the
    missing 'get_tipologia_config' import. This is to fix F821.
    """
    with app.app_context():
        db = get_db()
        cursor = db.cursor()
        ...
        cursor.execute("INSERT INTO proyectos (nombre, descripcion, creador_id) VALUES (?, ?, ?)",
                       ('Report Test Project', 'Desc', admin_id))
        project_id = cursor.lastrowid
        cursor.execute(
            "INSERT INTO conexiones (codigo_conexion, proyecto_id, tipo, subtipo, tipologia, solicitante_id, estado) "
            "VALUES (?, ?, 'MOMENTO', 'VIGA-COLUMNA (ALA)', 'T0', ?, 'SOLICITADO')",
            ('CONN-REPORT-TEST', project_id, admin_id)
        )
        connection_id = cursor.lastrowid
        db.commit()

    auth.login('admin', 'password')
    response = client.get(f'/conexiones/{connection_id}/reporte')
    # After fixing the NameError, this should return a standard HTML page.
    assert response.status_code == 200
    assert response.mimetype == 'text/html'
    assert b'Reporte de Conexi' in response.data  # "Reporte de Conexión"
    assert b'CONN-REPORT-TEST' in response.data
