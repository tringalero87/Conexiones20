...
class SQLiteDAL(BaseDAL):
    ...
    def get_conexion(self, conexion_id):
        ...
        conexion_query = """
            ...
            WHERE c.id = ?
        """
        return db.execute(conexion_query, (conexion_id,)).fetchone()
    ...
    def get_proyectos_for_user(self, user_id, is_admin):
        db = get_db()
        if is_admin:
            return db.execute("SELECT id, nombre FROM proyectos ORDER BY nombre").fetchall()
        else:
            return db.execute("""
    ...
    def get_proyecto(self, proyecto_id):
        db = get_db()
        return db.execute('SELECT * FROM proyectos WHERE id = ?', (proyecto_id,)).fetchone()

    def get_alias(self, nombre_perfil):
        db = get_db()
        return db.execute('SELECT alias FROM alias_perfiles WHERE nombre_perfil = ?', (nombre_perfil,)).fetchone()

    def get_all_aliases(self):
        db = get_db()
        return db.execute("SELECT alias, nombre_perfil FROM alias_perfiles ORDER BY nombre_perfil").fetchall()

    def get_all_conexiones_codes(self):
        db = get_db()
        return set(row['codigo_conexion'] for row in db.execute("SELECT codigo_conexion FROM conexiones").fetchall())

    def get_archivos_by_conexion(self, conexion_id):
        db = get_db()
        return db.execute('SELECT a.*, u.nombre_completo as subido_por FROM archivos a JOIN usuarios u ON a.usuario_id = u.id WHERE a.conexion_id = ? ORDER BY a.fecha_subida DESC', (conexion_id,)).fetchall()

    def get_comentarios_by_conexion(self, conexion_id):
        db = get_db()
        return db.execute("SELECT c.*, u.nombre_completo FROM comentarios c JOIN usuarios u ON c.usuario_id = u.id WHERE c.conexion_id = ? ORDER BY c.fecha_creacion DESC", (conexion_id,)).fetchall()

    def get_historial_by_conexion(self, conexion_id):
        db = get_db()
        return db.execute("SELECT h.*, u.nombre_completo FROM historial_estados h JOIN usuarios u ON h.usuario_id = u.id WHERE h.conexion_id = ? ORDER BY h.fecha DESC", (conexion_id,)).fetchall()

    def get_usuario_a_asignar(self, username):
        db = get_db()
        return db.execute('SELECT id, nombre_completo FROM usuarios WHERE username = ? AND activo = 1', (username,)).fetchone()
    ...
    def get_archivo(self, archivo_id, conexion_id):
        db = get_db()
        return db.execute('SELECT * FROM archivos WHERE id = ? AND conexion_id = ?', (archivo_id, conexion_id)).fetchone()
    ...
    def get_comentario(self, comentario_id, conexion_id):
        db = get_db()
        return db.execute('SELECT * FROM comentarios WHERE id = ? AND conexion_id = ?', (comentario_id, conexion_id)).fetchone()
    ...
    def get_users_for_notification(self, proyecto_id, roles_to_notify):
        ...
        """
        return db.execute(query, (proyecto_id, *roles_to_notify)).fetchall()
