...
def _generate_report_data_and_file(reporte_id, app_context):
    ...
    with app_context:
        ...
        if not reporte:
            current_app.logger.error(f"Reporte ID {reporte_id} no encontrado para generación programada.")
            return None, None, None, None

        try:
            filtros = json.loads(reporte['filtros'])
        except json.JSONDecodeError:
            current_app.logger.error(f"Error de JSON en filtros para reporte ID {reporte_id}.")
            return None, None, None, None
        ...
        try:
            if output_format == 'csv':
                ...
                rendered_html = render_template('email/reporte_programado.html',
            ...
            db.execute('UPDATE reportes SET ultima_ejecucion = CURRENT_TIMESTAMP WHERE id = ?', (reporte_id,))
            db.commit()
...
def scheduled_report_job(reporte_id):
    ...
    with current_app.app_context():
        ...
        if not reporte or not reporte['programado'] or not reporte['destinatarios']:
            current_app.logger.warning(f"Tarea programada para reporte ID {reporte_id} no ejecutada: reporte no encontrado, no programado o sin destinatarios.")
            return
        ...
        if file_content:
            try:
                ...
                msg.html = render_template('email/reporte_programado.html',
                                           reporte={'nombre': reporte['nombre'], 
                                                    'descripcion_filtros': reporte['descripcion'] if reporte['descripcion'] else 'Reporte generado automáticamente.'},
                ...
                mail.send(msg)
                current_app.logger.info(f"Reporte programado '{reporte['nombre']}' ({reporte_id}) enviado a {recipients}.")
            except Exception as e:
...
def eficiencia():
    ...
    processed_last_30d_row = db.execute("SELECT COUNT(id) as total FROM conexiones WHERE fecha_modificacion >= date('now', '-30 days') AND estado = 'APROBADO'").fetchone()
    processed_last_30d = processed_last_30d_row['total'] if processed_last_30d_row else 0

    total_approved_row = db.execute("SELECT COUNT(id) as total FROM conexiones WHERE estado = 'APROBADO'").fetchone()
    total_approved = total_approved_row['total'] if total_approved_row else 0

    total_rejected_history_row = db.execute("SELECT COUNT(DISTINCT conexion_id) as total FROM historial_estados WHERE estado = 'RECHAZADO'").fetchone()
    ...
    completed_by_user = db.execute("SELECT u.nombre_completo, COUNT(c.id) as total FROM conexiones c JOIN usuarios u ON c.realizador_id = u.id WHERE c.estado = 'APROBADO' AND c.fecha_modificacion >= date('now', '-30 days') GROUP BY u.id ORDER BY total DESC").fetchall()
    ...
    filters = {'start': (datetime.now() - timedelta(days=30)).strftime('%Y-%m-%d'), 'end': datetime.now().strftime('%Y-%m-%d')}
    
    slow_connections = db.execute("SELECT c.id, c.codigo_conexion, p.nombre as proyecto_nombre, u.nombre_completo as realizador_nombre, (julianday('now') - julianday(c.fecha_modificacion)) as dias_en_proceso FROM conexiones c JOIN proyectos p ON c.proyecto_id = p.id LEFT JOIN usuarios u ON c.realizador_id = u.id WHERE c.estado = 'EN_PROCESO' ORDER BY dias_en_proceso DESC LIMIT 5").fetchall()

    log_action('VER_EFICIENCIA', g.user['id'], 'sistema', None, "Visualizó el panel de eficiencia.")
...
def logs():
    ...
    logs_path = os.path.join(current_app.root_path, 'logs', 'heptaconexiones.log')
    log_entries = []
    try:
        ...
        flash("No se encontró el archivo de log. Puede que aún no se haya generado.", "info")
    
    log_action('VER_LOGS', g.user['id'], 'sistema', None, "Visualizó los logs del sistema.")
    return render_template('admin/logs.html', logs=log_entries, titulo="Logs del Sistema")
...
def clear_logs():
    ...
    try:
        with open(logs_path, 'w') as f:
            pass
        log_action('LIMPIAR_LOGS', g.user['id'], 'sistema', None, "Limpió el archivo de logs.")
        current_app.logger.warning(f"Admin '{g.user['username']}' ha limpiado el archivo de logs.")
        flash("El archivo de logs ha sido limpiado con éxito.", 'success')
    except Exception as e:
        current_app.logger.error(f"Error al intentar limpiar el archivo de logs: {e}")
...
def ver_auditoria():
    ...
    usuarios_para_filtro = db.execute('SELECT id, nombre_completo FROM usuarios ORDER BY nombre_completo').fetchall()
    acciones_para_filtro = db.execute('SELECT DISTINCT accion FROM auditoria_acciones ORDER BY accion').fetchall()
...
def storage_management():
    ...
    try:
        ...
        current_app.logger.error(f"Error al calcular el uso de almacenamiento: {e}")
    ...
    log_action('VER_ALMACENAMIENTO', g.user['id'], 'sistema', None, "Visualizó la gestión de almacenamiento.")
...
def _generate_computos_file(report_data, gran_total, file_format, filtros, proyecto_nombre_filtro):
    ...
    if file_format == 'xlsx':
        ...
        rendered_html = render_template('admin/reporte_computos_agregado.html',
...
def reporte_computos():
    ...
    if form_data and form.validate():
        ...
        if not conexiones and request.method == 'POST':
            flash("No se encontraron conexiones con datos computables para los filtros seleccionados.", "info")
            return redirect(url_for('admin.reporte_computos'))

        report_data = defaultdict(lambda: {'perfiles': defaultdict(lambda: {'cantidad': 0, 'peso_total': 0.0}), 'peso_total_proyecto': 0.0})
        gran_total_peso = 0.0

        def process_element(element, proyecto_nombre):
            nonlocal gran_total_peso
            if isinstance(element, dict) and 'perfil' in element and 'longitud' in element:
                nombre_perfil = element.get('perfil')
                longitud_mm = element.get('longitud')
                cantidad = element.get('cantidad', 1)
                if not nombre_perfil or not longitud_mm: return
                try:
                    longitud_mm = float(longitud_mm); cantidad = int(cantidad)
                except (ValueError, TypeError): return

                peso_total_elemento = calcular_peso_perfil(nombre_perfil, longitud_mm) * cantidad
        ...
        for conexion in conexiones:
            try:
                ...
                for value in detalles.values():
                    if isinstance(value, dict): process_element(value, conexion['proyecto_nombre'])
                    elif isinstance(value, list):
                        for item in value: process_element(item, conexion['proyecto_nombre'])
        ...
        if proyecto_id != 0:
            p = db.execute('SELECT nombre FROM proyectos WHERE id = ?', (proyecto_id,)).fetchone()
            if p: proyecto_nombre_filtro = p['nombre']

        file_format = request.args.get('format')
        if file_format in ['pdf', 'xlsx']:
            filename, mimetype, content = _generate_computos_file(dict(report_data), gran_total_peso, file_format, form.data, proyecto_nombre_filtro)
            if content:
        ...
        if request.method == 'POST':
            log_action('GENERAR_REPORTE_COMPUTOS', g.user['id'], 'reportes', proyecto_id, f"Generó reporte de cómputos para proyecto ID: {proyecto_id}")
            return render_template('admin/reporte_computos_agregado.html',
    ...
    log_action('VER_REPORTE_COMPUTOS_FORM', g.user['id'], 'reportes', None, "Visualizó el formulario de reporte de cómputos.")
    return render_template('admin/reporte_computos_form.html', form=form, titulo="Reporte de Cómputos Métricos")
...
def listar_reportes():
    ...
    db = get_db()
    reportes = db.execute("SELECT r.*, u.nombre_completo as creador_nombre FROM reportes r JOIN usuarios u ON r.creador_id = u.id ORDER BY r.nombre").fetchall()
    log_action('VER_REPORTES', g.user['id'], 'reportes', None, "Visualizó la lista de reportes.")
...
def nuevo_reporte():
    ...
    realizadores = db.execute("SELECT u.id, u.nombre_completo FROM usuarios u JOIN usuario_roles ur ON u.id = ur.usuario_id JOIN roles r ON ur.rol_id = r.id WHERE r.nombre = 'REALIZADOR' ORDER BY u.nombre_completo").fetchall()
    form.realizador_id.choices = [(0, 'Todos los Realizadores')] + [(r['id'], r['nombre_completo']) for r in realizadores]
    ...
    if form.validate_on_submit():
        ...
        cursor = db.execute("INSERT INTO reportes (nombre, descripcion, creador_id, filtros, programado, frecuencia, destinatarios) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (form.nombre.data, form.descripcion.data, g.user['id'], filtros_json, form.programado.data, form.frecuencia.data, form.destinatarios.data))
        ...
        if form.programado.data:
            ...
            if form.frecuencia.data in interval_map:
                try:
                    ...
                    current_app.logger.info(f"Reporte '{form.nombre.data}' (ID: {new_report_id}) programado con éxito.")
                except Exception as e:
                    current_app.logger.error(f"Error al programar el reporte '{form.nombre.data}': {e}", exc_info=True)
                    flash(f"Error al programar el reporte: {e}. El reporte se guardó pero no se pudo programar.", "warning")
...
def editar_reporte(reporte_id):
    ...
    if request.method == 'GET':
        ...
        if filtros.get('fecha_inicio'):
            form.fecha_inicio.data = datetime.strptime(filtros.get('fecha_inicio'), '%Y-%m-%d').date()
        if filtros.get('fecha_fin'):
            form.fecha_fin.data = datetime.strptime(filtros.get('fecha_fin'), '%Y-%m-%d').date()
        ...
        db.execute(
            ...
            (form.nombre.data, form.descripcion.data, filtros_json, form.programado.data, form.frecuencia.data, form.destinatarios.data, reporte_id)
        )
        db.commit()
        ...
        if form.programado.data:
            ...
            if form.frecuencia.data in interval_map:
                try:
                    ...
                    current_app.logger.info(f"Reporte '{form.nombre.data}' (ID: {reporte_id}) reprogramado con éxito.")
                except Exception as e:
                    current_app.logger.error(f"Error al reprogramar el reporte '{form.nombre.data}': {e}", exc_info=True)
                    flash(f"Error al reprogramar el reporte: {e}. El reporte se guardó pero no se pudo programar.", "warning")
        else:
            if current_app.scheduler.get_job(job_id):
                try:
                    current_app.scheduler.remove_job(job_id)
                    current_app.logger.info(f"Job de reporte '{job_id}' desprogramado.")
                except Exception as e:
...
def eliminar_reporte(reporte_id):
    ...
    if reporte:
        ...
        flash(f"El reporte '{reporte['nombre']}' ha sido eliminado.", 'success')
    else:
...
def ejecutar_reporte(reporte_id):
    ...
    if not file_content:
        flash("No se pudo generar el reporte. Verifique la configuración o los datos.", 'danger')
        return redirect(url_for('admin.listar_reportes'))
...
def configuracion():
    ...
    if form.validate_on_submit():
        old_per_page = db.execute("SELECT valor FROM configuracion WHERE clave = 'PER_PAGE'").fetchone()
        old_maintenance = db.execute("SELECT valor FROM configuracion WHERE clave = 'MAINTENANCE_MODE'").fetchone()
        
        db.execute("INSERT OR REPLACE INTO configuracion (clave, valor) VALUES (?, ?)", ('PER_PAGE', str(form.per_page.data)))
        db.execute("INSERT OR REPLACE INTO configuracion (clave, valor) VALUES (?, ?)", ('MAINTENANCE_MODE', '1' if form.maintenance_mode.data else '0'))
        db.commit()

        changes = {}
        if old_per_page and str(form.per_page.data) != old_per_page['valor']:
            changes['per_page'] = {'old': old_per_page['valor'], 'new': str(form.per_page.data)}
        if old_maintenance and ('1' if form.maintenance_mode.data else '0') != old_maintenance['valor']:
            changes['maintenance_mode'] = {'old': old_maintenance['valor'], 'new': ('1' if form.maintenance_mode.data else '0')}
        ...
        per_page_row = db.execute("SELECT valor FROM configuracion WHERE clave = 'PER_PAGE'").fetchone()
        maintenance_row = db.execute("SELECT valor FROM configuracion WHERE clave = 'MAINTENANCE_MODE'").fetchone()
        
        form.per_page.data = int(per_page_row['valor']) if per_page_row and per_page_row['valor'].isdigit() else current_app.config.get('PER_PAGE', 10)
        form.maintenance_mode.data = maintenance_row['valor'] == '1' if maintenance_row else False
    
    log_action('VER_CONFIGURACION', g.user['id'], 'sistema', None, "Visualizó la configuración del sistema.")
