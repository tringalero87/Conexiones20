    ...
    try:
        ...
        summary_query = f"""
            SELECT
                ...
                SUM(CASE WHEN solicitante_id = {p} AND estado = 'EN_PROCESO' THEN 1 ELSE 0 END)
                    as conexiones_en_proceso_solicitadas,
                SUM(CASE WHEN solicitante_id = {p} AND estado = 'APROBADO' THEN 1 ELSE 0 END)
                    as conexiones_aprobadas_solicitadas,
                SUM(CASE WHEN realizador_id = {p} AND estado = 'EN_PROCESO' THEN 1 ELSE 0 END)
                    as mis_tareas_en_proceso,
                SUM(CASE WHEN realizador_id = {p} AND estado = 'REALIZADO'
                    AND fecha_modificacion >= {date_func_30_days} THEN 1 ELSE 0 END)
                    as mis_tareas_realizadas_ult_30d,
                SUM(CASE WHEN aprobador_id = {p} AND estado = 'APROBADO'
                    AND fecha_modificacion >= {date_func_30_days} THEN 1 ELSE 0 END)
...
def _get_my_performance_data(db, user_id):
    ...
    try:
        if _is_testing():
            avg_time_sql = (
                f"SELECT AVG(julianday(h2.fecha) - julianday(h1.fecha)) as avg_days "
                f"FROM historial_estados h1 JOIN historial_estados h2 ON h1.conexion_id = h2.conexion_id "
                f"WHERE h1.usuario_id = {p} AND h1.estado = 'EN_PROCESO' AND h2.estado IN ('REALIZADO', 'APROBADO')"
            )
            completed_sql = (
                f"SELECT COUNT(id) as total FROM conexiones "
                f"WHERE (realizador_id = {p} AND estado = 'REALIZADO' AND "
                f"strftime('%Y-%m', fecha_modificacion) = strftime('%Y-%m', 'now')) OR "
                f"(aprobador_id = {p} AND estado = 'APROBADO' AND "
                f"strftime('%Y-%m', fecha_modificacion) = strftime('%Y-%m', 'now'))"
            )
        else:
            avg_time_sql = (
                f"SELECT AVG(EXTRACT(EPOCH FROM (h2.fecha - h1.fecha))) / 86400.0 as avg_days "
                f"FROM historial_estados h1 JOIN historial_estados h2 ON h1.conexion_id = h2.conexion_id "
                f"WHERE h1.usuario_id = {p} AND h1.estado = 'EN_PROCESO' AND h2.estado IN ('REALIZADO', 'APROBADO')"
            )
            completed_sql = (
                f"SELECT COUNT(id) as total FROM conexiones "
                f"WHERE (realizador_id = {p} AND estado = 'REALIZADO' AND "
                f"TO_CHAR(fecha_modificacion, 'YYYY-MM') = TO_CHAR(NOW(), 'YYYY-MM')) OR "
                f"(aprobador_id = {p} AND estado = 'APROBADO' AND "
                f"TO_CHAR(fecha_modificacion, 'YYYY-MM') = TO_CHAR(NOW(), 'YYYY-MM'))"
            )

        cursor.execute(avg_time_sql, (user_id,))
        avg_time_result = cursor.fetchone()
        if avg_time_result and avg_time_result['avg_days'] is not None:
            performance['avg_completion_time'] = f"{
                avg_time_result['avg_days']:.1f} días"
...
def _get_my_performance_chart_data(db, user_id):
    ...
    try:
        ...
        sql = (
            ...
            f"FROM conexiones WHERE ((realizador_id = {p} AND estado = 'REALIZADO') "
            f"OR (aprobador_id = {p} AND estado = 'APROBADO')) "
            f"AND fecha_modificacion >= {p} "
...
def _get_admin_dashboard_data(db, start_date, end_date):
    ...
    try:
        ...
        admin_data['kpis']['tiempo_aprobacion'] = f"{
            avg_days:.1f} días" if avg_days > 0 else "N/A"
...
def _get_user_tasks(db, user_id, user_roles):
    ...
    try:
        if 'APROBADOR' in user_roles:
            query = (
                f"SELECT c.id, c.codigo_conexion, p.nombre as proyecto_nombre, c.fecha_creacion, "
                f"c.tipo, c.estado, c.realizador_id FROM conexiones c JOIN proyectos p ON "
                f"c.proyecto_id = p.id JOIN proyecto_usuarios pu ON c.proyecto_id = pu.proyecto_id "
                f"WHERE c.estado = 'REALIZADO' AND pu.usuario_id = {p} "
                f"ORDER BY c.fecha_modificacion DESC LIMIT 5"
            )
            cursor.execute(query, (user_id,))
        ...
        if 'REALIZADOR' in user_roles:
            query = (
                f"SELECT c.id, c.codigo_conexion, p.nombre as proyecto_nombre, c.fecha_creacion, "
                f"c.tipo, c.estado, c.realizador_id FROM conexiones c JOIN proyectos p ON "
                f"c.proyecto_id = p.id WHERE c.estado = 'EN_PROCESO' AND c.realizador_id = {p} "
                f"ORDER BY c.fecha_modificacion DESC LIMIT 5"
            )
            cursor.execute(query, (user_id,))
        ...
        if 'SOLICITANTE' in user_roles:
            query = (
                f"SELECT id, codigo_conexion, estado, fecha_creacion, tipo FROM conexiones "
                f"WHERE solicitante_id = {p} AND estado NOT IN ('APROBADO') "
                f"ORDER BY fecha_creacion DESC LIMIT 5"
            )
            cursor.execute(query, (user_id,))
...
def dashboard():
    ...
    if 'REALIZADOR' in user_roles or 'APROBADOR' in user_roles:
        dashboard_data['my_performance'] = _get_my_performance_data(
            db, user_id)
        dashboard_data['my_performance_chart'] = _get_my_performance_chart_data(
            db, user_id)
    dashboard_data['my_projects_summary'] = _get_my_projects_summary(
        db, user_id)

    cursor = db.cursor()
    try:
        cursor.execute(
            f'SELECT widgets_config FROM user_dashboard_preferences WHERE usuario_id = {p}',
            (user_id,
             ))
        ...
        if 'ADMINISTRADOR' in user_roles:
            ...
            end_date_obj = datetime.strptime(
                date_end_str, '%Y-%m-%d') + timedelta(days=1)
            admin_data = _get_admin_dashboard_data(
...
def catalogo():
    ...
    try:
        if 'ADMINISTRADOR' in user_roles:
            ...
            cursor.execute(
                f"SELECT p.id, p.nombre FROM proyectos p JOIN proyecto_usuarios pu ON p.id = pu.proyecto_id WHERE pu.usuario_id = {p} ORDER BY p.nombre",
                (g.user['id'],
                 ))
...
def buscar():
    ...
    try:
        if _is_testing():
            ...
            for word in words:
                clauses.append(
                    f"(c.codigo_conexion LIKE {p} OR c.descripcion LIKE {p})")
            ...
            update_sql = (
                f"UPDATE conexiones SET fts_document = to_tsvector('simple', codigo_conexion || ' ' || "
                f"COALESCE(descripcion, '')) WHERE id IN (SELECT id FROM conexiones WHERE fts_document IS NULL "
                f"OR codigo_conexion || ' ' || COALESCE(descripcion, '') != fts_document)"
            )
            ...
            sql = f"""
                SELECT c.*, p.nombre as proyecto_nombre,
                       ...
                       ts_rank(to_tsvector('simple', c.codigo_conexion || ' ' ||
                               ...
                               to_tsquery('simple', {p})) as rank
                FROM conexiones c
                JOIN proyectos p ON c.proyecto_id = p.id
                LEFT JOIN usuarios sol ON c.solicitante_id = sol.id
                WHERE to_tsvector('simple', c.codigo_conexion || ' ' ||
                                  COALESCE(c.descripcion, '')) @@ to_tsquery('simple', {p})
                ORDER BY rank DESC
            """
