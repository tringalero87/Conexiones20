...
def login():
    ...
    if form.validate_on_submit():
        ...
        password_hash_to_check = user['password_hash'] if user else generate_password_hash(
            "dummy_password_for_timing_attack_mitigation")
        ...
        if error is None and user:
            ...
            log_action('INICIAR_SESION', user['id'], 'usuarios', user['id'],
                       f"Inicio de sesión exitoso para el usuario '{form.username.data}'.")
        ...
        log_action('INTENTO_FALLIDO_SESION', None, 'usuarios', None,
                   f"Intento de inicio de sesión fallido desde IP: {request.remote_addr}.")
    ...
    if form.validate_on_submit():
        try:
            ...
            if form.nombre_completo.data != g.user['nombre_completo'] or form.email.data != g.user['email']:
                dal.update_user_profile(
                    g.user['id'], form.nombre_completo.data, form.email.data)
                if form.nombre_completo.data != g.user['nombre_completo']:
                    changes['nombre_completo'] = {
                        'old': g.user['nombre_completo'], 'new': form.nombre_completo.data}
                if form.email.data != g.user['email']:
                    changes['email'] = {
                        'old': g.user['email'], 'new': form.email.data}
            ...
            if initial_email_notif_estado != form.email_notif_estado.data:
                dal.upsert_notification_preferences(
                    g.user['id'], form.email_notif_estado.data)
                changes['email_notif_estado'] = {
                    'old': initial_email_notif_estado, 'new': form.email_notif_estado.data}
            ...
            current_app.logger.error(
                f"Error al actualizar el perfil del usuario {g.user['id']}: {e}")
