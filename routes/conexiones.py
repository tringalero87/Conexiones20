...
def crear_conexion_form():
    ...
    tipologia_seleccionada = cs.get_tipologia_config(tipo, subtipo, tipologia_nombre)
...
def editar_conexion(conexion_id):
    ...
    tipologia_config = cs.get_tipologia_config(conexion['tipo'], conexion['subtipo'], conexion['tipologia'])
    ...
    if form.validate_on_submit():
        success, message, flash_message = cs.update_connection(conexion_id, form, g.user, session.get('user_roles', []))
        if success:
            flash(message, 'success')
            if flash_message:
    ...
    if request.method == 'GET':
        ...
        for i in range(1, num_perfiles + 1):
            if hasattr(form, f'perfil_{i}'):
                getattr(form, f'perfil_{i}').data = details['detalles'].get(f'Perfil {i}', '')
...
def asignar_realizador(conexion_id):
    ...
    success, message = cs.assign_realizador(conexion_id, username_a_asignar, g.user)
    if success:
        flash(message, 'success')
    else:
        flash(message, 'danger')
...
def eliminar_archivo(conexion_id, archivo_id):
    success, message = fs.delete_file(conexion_id, archivo_id, g.user, session.get('user_roles', []))
    if success:
        flash(message, 'success')
    else:
        flash(message, 'danger')
...
def eliminar_comentario(conexion_id, comentario_id):
    success, message = comment_s.delete_comment(conexion_id, comentario_id, g.user['id'])
    if success:
        flash(message, 'success')
    else:
        flash(message, 'danger')
