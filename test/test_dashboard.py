...
def test_dashboard_approver_isolation(client, app, auth):
    ...
    with app.app_context():
        ...
        cursor = db.cursor()
        cursor.execute(
            "INSERT INTO proyectos (nombre, descripcion) VALUES (?, ?)", ('Proyecto A', 'Desc A'))
        project_a_id = cursor.lastrowid
        cursor.execute(
            "INSERT INTO proyectos (nombre, descripcion) VALUES (?, ?)", ('Proyecto B', 'Desc B'))
    ...
    with app.app_context():
        ...
        cursor.execute(
            "INSERT INTO usuario_roles (usuario_id, rol_id) VALUES (?, (SELECT id FROM roles WHERE nombre = 'APROBADOR'))", (approver_a_id,))
        cursor.execute("INSERT INTO proyecto_usuarios (proyecto_id, usuario_id) VALUES (?, ?)",
                       (project_a_id, approver_a_id))
        ...
        cursor.execute(
            "INSERT INTO usuario_roles (usuario_id, rol_id) VALUES (?, (SELECT id FROM roles WHERE nombre = 'APROBADOR'))", (approver_b_id,))
        cursor.execute("INSERT INTO proyecto_usuarios (proyecto_id, usuario_id) VALUES (?, ?)",
                       (project_b_id, approver_b_id))
        ...
        cursor.execute(
            "INSERT INTO usuario_roles (usuario_id, rol_id) VALUES (?, (SELECT id FROM roles WHERE nombre = 'SOLICITANTE'))", (requester_id,))
...
def multi_role_user(app):
    ...
    with app.app_context():
        ...
        cursor.execute(
            ...
            ('multi_role', generate_password_hash('password'),
             'Multi Role User', 'multi@test.com', 1)
        )
        ...
        cursor.execute(
            ...
            ('MULTI-005', project_id, 1, user_id,
...
def test_dashboard_summary_kpis_for_multi_role_user(client, auth, multi_role_user):
    ...
    def check_kpi(icon_class, label, value):
        ...
        assert re.search(pattern, response_data,
                         re.DOTALL), f"KPI '{label}' con valor '{value}' no encontrado."
